	...
	IRepeat(IBlock *block, string *var, int myline) : IStatement(myline){
		...
		if(IMath::isNumber(*varname)){
			if(IMath::isInteger(*varname))
				...
				//warning
				cout << "Atencao: a utilizacao de um valor real em uma expressao \'repita\' causa arredondamento para cima (i.e, `repita 3.3 vezes` repetiria o bloco de código 4 vezes)." << endl;
			}
